<template>
    ...
    <div class="flex flex-col w-full h-full bg-[#F0F9E3] items-center justify-between gap-4 text-center">
        <div class="flex flex-col w-full h-full justify-center items-center p-4">
            <div v-if="locationData.data" class="w-fit p-4 flex flex-col h-fit gap-4 border-2 border-[#D9D9D9] rounded-[8px] text-[#031C1A]">
                <div class="flex gap-2 justify-center items-center">
                    <img src="../../public/globeIcon.png" class="size-12" />
                    <h1 class="md:text-[32px] text-[24px]">{{locationData.data.name}}</h1>
                </div>
                ...
                <div class="flex  justify-around items-center gap-4  w-full h-full">
                    ...
                    <div class="flex flex-col justify-center items-center gap-0">
                        <h1 class="md:text-[24px] text-[16px] text-[rgba(3,28,26,0.6)]">Dimension</h1>
                        <h1 class="md:text-[32px] text-[24px]">{{locationData.data.dimension}}</h1>
                    </div>
                    <div class="h-full w-0.5 bg-[#D9D9D9] rounded-full"></div>
                    <div class="flex flex-col justify-center items-center gap-0">
                        <h1 class="md:text-[24px] text-[16px] text-[rgba(3,28,26,0.6)]">Type</h1>
                        <h1 class="md:text-[32px] text-[24px]">{{locationData.data.type}}</h1>
                    </div>
        ...
        <div class="flex flex-col w-full gap-0 justify-center items-center"> 
            <h1 class="md:text-[32px] text-[24px] text-[#031c1a] flex justify-center items-center gap-2">Residents in location <span class="md:text-[20px] text-[12px]">({{ charactersData.data.length }})</span></h1>
...
let locationData = reactive({data: []})
...
async function getLocationData(){
    ...
    .then((response) => {
        ...
        locationData.data = response.data
...
async function getLocationsCharacters(){
    let characters = locationData.data.residents
    let temp = []
...
watch(()=> locationData.data, (newlocationData, oldlocationData)=>{
